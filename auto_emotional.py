...
with open("emotion.json","r",encoding="utf-8") as file:
    refer_path = json.load(file)
...
labels=['惊讶', '高兴', '悲伤', '恐惧', '厌恶', '愤怒', '喜好']
def handle_emotion(text:str):
    ...
    if score<0.5:
        ...
        label=labels[scores.index(score)]
        print(f"{label} {score}")
        return label

def get_refer(emotion:str):
    return random.choice(refer_path.get(emotion))
...
async def tts_endpoint(request: Request):
    ...
    if not refer_wav_path or not prompt_text or not prompt_language:
        emotion=handle_emotion(text)
        refer=get_refer(emotion)
        refer_wav_path=next(iter(refer.keys()))
        prompt_text=refer[refer_wav_path][0]
        prompt_language=refer[refer_wav_path][1]
        logger.info(f"自动选择参考音频路径：{refer_wav_path}")
        logger.info(f"自动选择参考音频文本：{prompt_text}")
    return handle(
        refer_wav_path,
        prompt_text,
        prompt_language,
